//! Game board.
//!
//! NOTE: This implementation is adapted based on  https://github.com/orhun/battleship-rs.

use super::ship::{Ship, ShipType};
...
impl Coordinate {
    pub fn new(x: u8, y: u8, is_hit: bool) -> Self {
        Self { x, y, is_hit }
    }
}
...
impl Grid {
    ...
    /// Marks a hit or a miss on the "hidden" grid (i.e. the opponent view).
    ///
    /// Note, that we are mis-using the [Self::ships] fields to be populated with
    /// solely the hits and nothing else.
    /// This way we can print the "hidden" map easily based on the same methods for
    /// the open map.
    pub fn mark_hit(&mut self, coord: &Coordinate) -> eyre::Result<()> {
        if self.ships.iter().any(|ship| ship.coords.contains(coord)) {
            return Err(eyre::eyre!("coordinate already hit"));
        };

        if coord.x > self.width || coord.x > self.height {
            return Err(eyre::eyre!("coordinate outside of grid"));
        };

        self.ships.push(Ship {
            type_: ShipType::Boat,
            coords: vec![coord.to_owned()],
        });
        Ok(())
    }
    ...
    pub fn as_string(&self, show_ships: bool) -> eyre::Result<String> {
        let mut s = Vec::new();
...
mod tests {
    ...
    use crate::game::ship::Orientation;
